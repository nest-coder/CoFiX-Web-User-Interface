...
interface CoFiXV2ControllerInterface extends ethers.utils.Interface {
  ...
  encodeFunctionData(
    functionFragment: "calcGamma",
    values: [BigNumberish]
  ): string;
  encodeFunctionData(
    functionFragment: "getLatestPriceAndAvgVola",
    values: [string]
  ): string;
...
export class CoFiXV2Controller extends BaseContract {
  ...
  functions: {
    ...
    calcK(
      ...
      bn: BigNumberish,
      overrides?: CallOverrides
    ): Promise<[number] & { k: number }>;

    calcGamma(
      vola: BigNumberish,
      overrides?: CallOverrides
    ): Promise<[BigNumber] & { gamma: BigNumber }>;
  ...
  filters: {
    NewCGamma(
      token?: null,
      gamma?: null
    ): TypedEventFilter<[string, number], { token: string; gamma: number }>;
